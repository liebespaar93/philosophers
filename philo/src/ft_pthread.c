...
void	ft_right_handed(t_philo *philo, t_man *man)
{
	while (philo->time_to_die)
	{
		...
		pthread_mutex_lock(&philo->forks[(man->id + 1) % philo->number_of_philosophers]);
		ft_printf_mutex(man, ft_real_time(man), "has taken a fork");
		...
		pthread_mutex_unlock(&philo->forks[(man->id + 1) % philo->number_of_philosophers]);
...
void	ft_ㅣeft_handed(t_philo *philo, t_man *man)
{
	while (philo->time_to_die)
	{
		pthread_mutex_lock(&philo->forks[(man->id + 1) % philo->number_of_philosophers]);
